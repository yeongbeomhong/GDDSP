...
if config.input_dist_amount_random == True:
    experiment_full_name =  config.experiment_name + \
        f"Test_D{D}DL{DL}AR{AR}RE{RE}_nHarm{config.n_harmonics}_finalthesis_noWH_trainGuitarSet_testIDMT"
else:
    experiment_full_name =  config.experiment_name + \
        f"Test_D{D}DL{DL}AR{AR}RE{RE}__nHarm{config.n_harmonics}_finalthesis_noWH_trainGuitarSet_testIDMT"
...
print(f"Two test_dataloader with Real-World Pedal, {dist_a}, {dist_b}, {dist_c}  and {dist_d} has been built.")
...
print(f"Counts of test audio-{dist_c} exceprts with 2-sec: {len(test_dataset_c)}")
print(f"Counts of test audio-{dist_d} exceprts with 2-sec: {len(test_dataset_d)}")

# S2. Load the Structure of GDDSP without decoding-preeq-tanh layers for distortion
...
if config.use_room_acoustic == True:
    signal_key = "audio_room" # There always no distorted recon audio "audio_dist" in GDDSP loaded in this file.
    de_reverb_signal_key = "audio_dry"
else:
    signal_key = "audio_dry"
    de_reverb_signal_key = None
...
with torch.no_grad(): # Test doesnt need grad graph
    ...
    wandb.log({f"(without WH_Dist, {dist_a}) wet_test_loss": avg_dist_recon_loss_a,
...
if de_reverb_signal_key != None :
    sample_recon_test_audio_a_1_de_reverb = audio_recon[de_reverb_signal_key][0].detach().cpu().numpy()
    sample_recon_test_audio_a_2_de_reverb = audio_recon[de_reverb_signal_key][1].detach().cpu().numpy()
    sf.write(audio_path + f"/recon_{dist_a}_dereverb_1.wav", sample_recon_test_audio_a_1_de_reverb, sr)
    sf.write(audio_path + f"/recon_{dist_a}_dereverb_2.wav", sample_recon_test_audio_a_2_de_reverb, sr)
...
with torch.no_grad(): # Test doesnt need grad graph
    ...
    wandb.log({f"(without WH_Dist, {dist_b}) wet_test_loss": avg_dist_recon_loss_b,
            "num_trainable_params" : num_trainable_parameters})
...
if de_reverb_signal_key != None :
    sample_recon_test_audio_b_1_de_reverb = audio_recon[de_reverb_signal_key][0].detach().cpu().numpy()
    sample_recon_test_audio_b_2_de_reverb = audio_recon[de_reverb_signal_key][1].detach().cpu().numpy()
    sf.write(audio_path + f"/recon_{dist_b}_dereverb_1.wav", sample_recon_test_audio_a_1_de_reverb, sr)
    sf.write(audio_path + f"/recon_{dist_b}_dereverb_2.wav", sample_recon_test_audio_a_2_de_reverb, sr)
...
with torch.no_grad(): # Test doesnt need grad graph
    ...
    wandb.log({f"(without WH_Dist, {dist_c}) wet_test_loss": avg_dist_recon_loss_c,
            "num_trainable_params" : num_trainable_parameters})
...
if de_reverb_signal_key != None :
    sample_recon_test_audio_c_1_de_reverb = audio_recon[de_reverb_signal_key][0].detach().cpu().numpy()
    sample_recon_test_audio_c_2_de_reverb = audio_recon[de_reverb_signal_key][1].detach().cpu().numpy()
    sf.write(audio_path + f"/recon_{dist_c}_dereverb_1.wav", sample_recon_test_audio_c_1_de_reverb, sr)
    sf.write(audio_path + f"/recon_{dist_c}_dereverb_2.wav", sample_recon_test_audio_c_2_de_reverb, sr)
...
with torch.no_grad(): # Test doesnt need grad graph
    ...
    wandb.log({f"(without WH_Dist, {dist_d}) wet_test_loss": avg_dist_recon_loss_d,
            "num_trainable_params" : num_trainable_parameters})
...
if de_reverb_signal_key != None :
    sample_recon_test_audio_d_1_de_reverb = audio_recon[de_reverb_signal_key][0].detach().cpu().numpy()
    sample_recon_test_audio_d_2_de_reverb = audio_recon[de_reverb_signal_key][1].detach().cpu().numpy()
    sf.write(audio_path + f"/recon_{dist_d}_dereverb_1.wav", sample_recon_test_audio_d_1_de_reverb, sr)
    sf.write(audio_path + f"/recon_{dist_d}_dereverb_2.wav", sample_recon_test_audio_d_2_de_reverb, sr)
